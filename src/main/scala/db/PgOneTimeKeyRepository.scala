...
class PgOneTimeKeyRepository(xa: Transactor[IO]) extends OneTimeKeyRepository[IO] {
  ...
  override def findAllByDevice(appId: String, userId: String, deviceId: String): IO[List[OneTimeKey]] =
    sql"""select app_id, user_id, device_id, id, key from one_time_keys where app_id=$appId::uuid and user_id=$userId and device_id=$deviceId"""
      .query[OneTimeKey].to[List].transact(xa)
